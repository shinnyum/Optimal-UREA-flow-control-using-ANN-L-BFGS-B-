...
h2o.download_pojo(EFF.h2o)

first.Layer.Weights <- h2o.weights(EFF.h2o, matrix_id = 1)
second.Layer.Weights <- h2o.weights(EFF.h2o, matrix_id = 2)
first.Layer.Biases <- h2o.biases(EFF.h2o, vector_id = 1)
second.Layer.Biases <- h2o.biases(EFF.h2o, vector_id = 2)
...
X.input <- as.matrix(subset(test.data.frame, select=c(-BoilerEFF)))
...
cbind(as.data.frame(predicted.EFF.h2o$predict), inverseTransform.normalized.Y.output)
