...
h2o.download_pojo(MSF.h2o)

first.Layer.Weights <- h2o.weights(MSF.h2o, matrix_id = 1)
second.Layer.Weights <- h2o.weights(MSF.h2o, matrix_id = 2)
first.Layer.Biases <- h2o.biases(MSF.h2o, vector_id = 1)
second.Layer.Biases <- h2o.biases(MSF.h2o, vector_id = 2)
...
second.Layer.Biases_Num.vector <- as.matrix(as.numeric(second.Layer.Biases))

## Define and initialize input dataset
X.input <- as.matrix(subset(test.data.frame, select=c(-MAIN_STEAM_FLOW_SUM)))
...
cbind(as.data.frame(predicted.MSF.h2o$predict), inverseTransform.normalized.Y.output)
lines(inverseTransform.normalized.Y.output, col="green",lwd=2)

print("ANN-formula Extraction is done")
